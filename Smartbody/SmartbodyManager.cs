...
public class SmartbodyManager : ICharacterController
{
    ...
    protected Dictionary<string, SmartbodyGestureMapDefinition> m_gestureMapDefinitions = new Dictionary<string, SmartbodyGestureMapDefinition>();
    ...
    public void Init(SmartbodyInit initSettings)
    {
        ...
        if (string.IsNullOrEmpty(initSettings.initialPySeqPath))
            pySeqPath = VHUtils.GetExternalAssetsPath() + "SB";
        else
            pySeqPath = initSettings.initialPySeqPath;

        if (string.IsNullOrEmpty(initSettings.audioPath))
        {
            initSettings.audioPath = VHUtils.GetExternalAssetsPath() + "Sounds";
        }
    ...
    public virtual void AddGestureMapDefinition(SmartbodyGestureMapDefinition gestureMap)
    {
    ...
    public void SaveAllMotions(string path)
    {
        ...
        foreach (SmartbodyMotion motion in allMotions)
        {
            ...
            {
                File.Move(motion.MotionName, motion.MotionName + ".skm");
            }
            else
            {
    ...
    public void MapSkeletonAndAssetPaths(SmartbodyJointMap jointMap, string skeletonName, List<KeyValuePair<string, string>> assetPaths)
    {
        ...
        {
            string[] files = VHUtils.GetStreamingAssetsFiles(path.Value, ".skm");
    ...
    public virtual void LoadAssetPaths(List<KeyValuePair<string, string>> assetPaths)
    {
        ...
        if (assetPaths != null)
        {
            foreach (var pair in assetPaths)
            {
                ...
                // load all .sk's
                {
                    string[] files = VHUtils.GetStreamingAssetsFiles(pathName, ".sk");
                    foreach (var file in files)
                    {
                        ...
                        byte[] skeletonByte = VHUtils.LoadStreamingAssetsBytes(file);
                ...
                // load all .skm's
                {
                    string[] files = VHUtils.GetStreamingAssetsFiles(pathName, ".skm");
                    foreach (var file in files)
                    {
                        string motionName = Path.GetFileNameWithoutExtension(file);
                        ...
                        byte[] motionByte = VHUtils.LoadStreamingAssetsBytes(file);
                        if (motionByte != null)
                        {
        ...
        if (sbmCharacters != null)
        {
            for (int i = 0; i < sbmCharacters.Length; i++)
            {
                ...
                if (init != null)
                {
                    ...
                    SmartbodyGestureMapDefinition gestureMap = GetComponent<SmartbodyGestureMapDefinition>();   // ok if it's null
    ...
    public void SendBmlReply(string characterName, string requestId, string utteranceId)
    {
        ...
        else
        {
            ...
            if (audioFile == null)
            {
                ...
                string audioPath = VHUtils.GetStreamingAssetsPath() + "Sounds";
                SmartbodyInit initSettings = GetComponent<SmartbodyInit>();
                if (initSettings != null)
                {
                ...
                audioPath += ("/" + utteranceId + ".bml");
#if !UNITY_WEBPLAYER
                if (File.Exists(audioPath))
                {
                    string bmlText = File.ReadAllText(audioPath);
                    SendBmlReply(characterName, requestId, utteranceId, bmlText);
                }
                else
                {
                    //Debug.LogError("Failed to SendBmlReply for utterance: " + utteranceId + " character: " + characterName);
                    string bmlText = string.Format("ERROR - audio file '{0}' not found, try backup voice", audioPath);
                    SendBmlReply(characterName, requestId, utteranceId, bmlText);
                }
#endif
            }
            ...
            {
                // utterance was found
                string bmlText = audioFile.m_LipSyncInfo.text;
                SendBmlReply(characterName, requestId, utteranceId, bmlText);
            }
        }
    }
    ...
    public string GetAnimFromGestureData(string sbmCharName, string lexeme, string type)
    {
        ...
        SmartbodyGestureMapDefinition gestureMap = character.GetComponent<SmartbodyGestureMapDefinition>();
        if (gestureMap == null)
        {
    ...
    public void SBPlayFAC(string character, int au, FaceSide side, float weight, float duration, float ready, float relaxTime)
    {
        // side == "left", "right" or "both"
    ...
    public override void SBGaze(string character, string gazeAt, string targetBone, SmartbodyManager.GazeDirection gazeDirection,
    ...
    public override void SBExpress(string character, string uttID, string uttNum, string text, string target)
    {
        ...
        string uttNumModified = uttNum;
        if (string.IsNullOrEmpty(uttNum))
            uttNumModified = DateTime.Now.ToString("yyyyMMddHHmmssffff");

        string message = string.Format("vrExpress {0} {4} 1303332588320-{2}-1 <?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\" ?>"
            ...
            + "<bml><speech id=\"sp1\" ref=\"{1}\" type=\"application/ssml+xml\">{3}</speech></bml></act>", character, uttIDModified, uttNumModified, text, target);
